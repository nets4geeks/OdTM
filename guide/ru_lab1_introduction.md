...
Коммуникации (потоки данных) между элементами системы осуществляются посредством сетевых технологий: пользователь (браузер) и веб-приложение взаимодействуют по протоколу HTTPS,
а веб-приложение с базой данных - по прикладному протоколу, поддерживающему SQL.
...
Процессы на диаграммах изображают как окружности, хранилища - в виде двух параллельных линий, внешние сущности - прямоугольники, а потоки данных - линии со стрелками.
...
Сетевой сеанс является двунаправленным, т.е. нет необходимости создавать поток от сервера к клиенту (однако данный тип трафика должен учитываться).
...
В этом руководстве используется предметно-ориентированная модель угроз облачным системам, основанная на [соответствующем каталоге шаблонов угроз](https://nets4geeks.github.io/acctp/).

Используя данную модель, можно трансформировать общий пример в облачный. Для этого (см. скриншот ниже) элементы диаграммы 'app' и 'db' следует пометить как
'class#CloudApplication' (облачное приложение) , а 'user' - как 'class#RemoteUser' (удаленный пользователь).
Концепты облачного приложения и удаленного пользователя входят в состав предметно-ориентированной модели.


![lab1_example4](pics/lab1_example2.png)

После обработки утилитой OdTMServer к [элементам диаграммы](models/lab1_example2_modelled.json) (**Пример 2**) будут добавлены новые угрозы.
...
* Используя Threat Dragon, создайте диаграмму, которая включает веб-приложение (интерфейс и базу данных), опубликованное посредством HTTP-балансировщика.
Тщательно определите все потоки данных. Сохраните диаграмму как файл JSON.

* Используя Threat Dragon, добавьте противодействия к угрозам элементу 'user' из [примера 2](models/lab1_example2_modelled.json) и сохраните отчет как файл PDF.
Для нахождения противодействий используйте [каталог шаблонов угроз облачным системам](https://nets4geeks.github.io/acctp/catalog/) и другие источники.

## Вопросы для самопроверки

* Что такое моделирование угроз?
* Какие подходы используются для моделирования угроз?
* Из каких элементов может состоять диаграмма потоков данных?
* Что такое STRIDE?
